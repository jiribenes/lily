...
getFixity op = if isPostfixOp op then Postfix else Prefix  
  where
    isPostfixOp o = o `elem` [UnOpPreInc, UnOpPreDec]
...
isSubintervalOf (a, b) (c, d) | file a /= file c = False -- this should be impossible!
                              | offset a >= offset c && offset b <= offset d = True -- TODO: fix this! It seems to be inclusive for UnOps but exclusive for BinOps.
                              | otherwise = False
...
parseBinOp cursor = case tokensUnderCursorInInterval cursor interestingInterval of
...
getUnaryOpIntervalAndFixity :: T.CursorK 'UnaryOperator -> Cursor -> (UnaryFixity, (Location, Location))
getUnaryOpIntervalAndFixity cursor child 
  | startOffset cursorInterval == startOffset childInterval = (Postfix, (snd childInterval, snd cursorInterval))
  | endOffset cursorInterval == endOffset childInterval = (Prefix, (fst cursorInterval, fst childInterval))
...
tokensUnderCursorInInterval :: T.HasExtent k => T.CursorK k -> (Location, Location) -> [Token]
tokensUnderCursorInInterval cursor int = cursor & allTokens & wrapInInterval & filterInInterval <&> fst
  where
    ...
    wrapInInterval xs = zip xs $ xs <&> getLocationInterval . fromJust . tokenExtent

    filterInInterval :: [(Token, (Location, Location))] -> [(Token, (Location, Location))]
