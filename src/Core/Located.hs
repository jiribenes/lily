...
module Core.Located (Location(..), Located(..)) where

import Language.C.Clang.Location
import Core.Syntax
import Language.C.Clang.Cursor (cursorExtent, Cursor)
...
instance Ranged SourceRange where
    range = Range . sourceRangeToLocation
        where
            sourceRangeToLocation = (,) <$> (spellingLocation . rangeStart) <*> (spellingLocation . rangeEnd)
